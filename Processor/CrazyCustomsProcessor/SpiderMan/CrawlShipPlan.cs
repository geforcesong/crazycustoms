...
namespace SpiderMan
{
    public class CrawlShipPlan
    {
        /*
        #region Public
        #region 洋山

        public List<ShipPlanWebEntity> CrawlYang1()
        {

            for (int i = 1; i < 30; i++)
            {
                string strUrl = "http://www.shsict.com/as/query/search/comm/continplan.jsp?page=" + i;

                string content = ProcessorUtilities.HtmlParseUtils.SaveWebPage(strUrl);

                List<string> trList = ProcessorUtilities.HtmlParseUtils.GetSubStrings(content, "", null, "", null, null, null);

                foreach (string tr in trList)
                {
                    List<string> tdList = ProcessorUtilities.HtmlParseUtils.GetSubStrings(content, "", null, "", "", null, "");
                }
            }
        ...
        public List<ShipPlanWebEntity> CrawlYang3()
        {
            ...
            {
                List<ShipPlanWebEntity> lstEntity = new List<ShipPlanWebEntity>();
                    ...
                    if (!shipInfo.Contains("没有查到任何纪录"))
                    {
                        ...
                        if (trList != null && trList.Count > 0)
                        {
                            foreach (string tr in trList)
                            {
                                ...
                                {
                                    ShipPlanWebEntity entity = new ShipPlanWebEntity();
        ...
        public List<ShipPlanWebEntity> CrawlWai1()
        {
            //第一次取所有船名
        ...
        public List<ShipPlanWebEntity> CrawlWai2()
        {
        ...
        public List<ShipPlanWebEntity> CrawlWai4()
        {
        ...
        public List<ShipPlanWebEntity> CrawlWai5()
        {
        ...
        public List<ShipPlanWebEntity> CrawlLuoJing()
        {
        ...
        public List<ShipPlanWebEntity> CrawlZhangHuaBang()
        {
        ...
        private List<ShipPlanWebEntity> CrawlWaiData(string Url, string dock)
        {
            List<ShipPlanWebEntity> lstEntity = new List<ShipPlanWebEntity>();
            string content = ProcessorUtilities.HtmlParseUtils.SaveWebPage(Url, Encoding.UTF8);
